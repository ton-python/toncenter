...
| -------------- | ---------- | --------------------------------------------------------- |
| `api_key`      | Optional   | API key or list of keys for [rotation](/guide#key-rotation). |
...
<Note>
  Without an API key the SDK auto-limits to 1 RPS / 1.2 s on the client side to match the server-side throttle. With a key, client-side limiting is off unless set explicitly. The server enforces its own rate limits independently — see [API Key](/index#api-key).
</Note>
...
<Note>
  Keys on the same plan share a combined server-side RPS limit — rotation helps when keys have independent limits (different plans).
</Note>
