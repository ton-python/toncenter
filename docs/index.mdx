...
<Note>
  Multiple keys on the same plan share a combined RPS limit — for example, three Plus keys total 25 RPS, not 25 each. The SDK supports [key rotation](/reference#key-rotation) with per-key rate limits for REST and automatic reconnection on connection limit for streaming.
</Note>
